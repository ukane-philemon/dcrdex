...
export default class WalletsPage extends BasePage {
  ...
  async stepSubmit () {
    ...
    const res = await postJSON('/api/getsendFee', open)
    ...
    await this.hideBox()
    loaded()
    page.vSendFee.textContent = Doc.formatFullPrecision(res.fee, app().unitInfo(assetID))
    page.vSendDestinationAmt.textContent = Doc.formatFullPrecision(value - res.fee, app().unitInfo(assetID))
    ...
    if (!subtract) {
      ...
      page.vTotalSend.textContent = Doc.formatFullPrecision(value + res.fee, app().unitInfo(assetID))
      page.balanceAfterSend.textContent = Doc.formatFullPrecision(wallet.balance.available - (value + res.fee), app().unitInfo(assetID))
    }
    this.animation = this.showBox(page.vSendForm)
  }
