...
func (dcr *ExchangeWallet) OwnsAddress(address string) (bool, error) {
	...
	return dcr.wallet.AccountOwnsAddress(dcr.ctx, addr, dcr.acctName)
}
...
func (dcr *ExchangeWallet) Balance() (*asset.Balance, error) {
	...
	bal, err := dcr.wallet.AccountBalance(dcr.ctx, 0, dcr.acctName)
	if err != nil {
		return nil, err
	}
	return &asset.Balance{
		Available: toAtoms(bal.Spendable) - locked,
		Immature: toAtoms(bal.ImmatureCoinbaseRewards) +
			toAtoms(bal.ImmatureStakeGeneration),
		Locked: locked + toAtoms(bal.LockedByTickets),
	}, nil
}
...
func (dcr *ExchangeWallet) Unlock(pw []byte) error {
	...
	return dcr.wallet.UnlockAccount(dcr.ctx, pw, dcr.acctName)
}

// Lock locks the exchange wallet.
func (dcr *ExchangeWallet) Lock() error {
	return dcr.wallet.LockAccount(dcr.ctx, dcr.acctName)
}
...
func (dcr *ExchangeWallet) Withdraw(address string, value, feeRate uint64) (asset.Coin, error) {
	...
	msgTx, val, err := dcr.send(addr, value, dcr.feeRateWithFallback(feeRate), true)
	if err != nil {
		return nil, err
	}
	return newOutput(msgTx.CachedTxHash(), 0, val, wire.TxTreeRegular), nil
}
...
func (dcr *ExchangeWallet) Send(address string, value, feeRate uint64) (asset.Coin, error) {
	...
	msgTx, val, err := dcr.send(addr, value, dcr.feeRateWithFallback(feeRate), false)
	if err != nil {
		return nil, err
	}
	return newOutput(msgTx.CachedTxHash(), 0, val, wire.TxTreeRegular), nil
...
// send sends the amount to the address. If minusFees is false, fee is added
// to the amount.
func (dcr *ExchangeWallet) send(addr stdaddr.Address, val, feeRate uint64, minusFees bool) (*wire.MsgTx, uint64, error) {
	if val == 0 {
	...
	msgTx, val, err := dcr.sendCoins(addr, coins, val, feeRate, minusFees)
	if err != nil {
...
func (dcr *ExchangeWallet) sendRegFee(addr stdaddr.Address, regFee, netFeeRate uint64) (*wire.MsgTx, uint64, error) {
	...
	msgTx, val, err := dcr.sendCoins(addr, coins, regFee, netFeeRate, false)
	if err != nil {
		if retErr := dcr.returnCoins(coins); retErr != nil {
			return nil, 0, retErr
		}
